...
class AnomalyDetectionPipeline :
    ...
    def classification_trainer(self, x: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            self.save(self.path, all_models)
            return results
...
data=pd.read_csv('data/activities.csv',nrows=1,sep=';')
pipeline = AnomalyDetectionPipeline(columns=data.columns,path='models/xprocessor.pkl')
resulta=pipeline.final_predection(data)
pprint(resulta)